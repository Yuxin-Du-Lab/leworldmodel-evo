...
def _process_parquet_file_worker(args):
    ...
    try:
        ...
        for i in range(len(df) - action_horizon):
            ...
            future_row = df.iloc[i + action_horizon]
            video_paths = {}
            ...
            episode = {
                ...
                "future_timestamp": future_row.get("timestamp", None),
            }
...
class LeRobotDataset(Dataset):
    def __init__(
        ...
        self.pixel_view_transform = T.Compose([
            T.Resize((self.image_size, self.image_size), interpolation=InterpolationMode.BICUBIC),
            T.ToTensor()
        ])
        self.pixel_merge_transform = T.Resize((self.image_size, self.image_size), interpolation=InterpolationMode.BICUBIC)
    ...
    def _transform_single_view_for_pixels(self, img: Image.Image) -> torch.Tensor:
        return self.pixel_view_transform(img)

    def _stitch_views_horizontally(self, images: List[torch.Tensor]) -> torch.Tensor:
        if len(images) == 0:
            return torch.zeros(3, self.image_size, self.image_size)

        merged = torch.cat(images, dim=2)
        return self.pixel_merge_transform(merged)

    def _build_pixels_pair(self, video_paths: dict, timestamp: float, future_timestamp: float) -> torch.Tensor:
        current_frames = self._load_video_frame(video_paths, timestamp)
        future_frames = self._load_video_frame(video_paths, future_timestamp)

        current_views = [self._transform_single_view_for_pixels(img) for img in current_frames]
        future_views = [self._transform_single_view_for_pixels(img) for img in future_frames]

        current_merged = self._stitch_views_horizontally(current_views)
        future_merged = self._stitch_views_horizontally(future_views)

        return torch.stack([current_merged, future_merged], dim=0)
    ...
    def __getitem__(self, idx):
        ...
        future_timestamp = item.get("future_timestamp", None)
        if future_timestamp is None:
            raise KeyError(
                f"missing future_timestamp in cache file {cache_filepath}; clear stale cache and rebuild"
            )

        pixels = self._build_pixels_pair(
            item["video_paths"],
            item["timestamp"],
            future_timestamp,
        )
